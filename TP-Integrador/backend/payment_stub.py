...
async def process_payment(
    user_id: int, amount: float, payment_method: str = "credit_card"
) -> Dict:
    """
    ...
    """
    await asyncio.sleep(
        random.uniform(MIN_PAYMENT_LATENCY_SECONDS, MAX_PAYMENT_LATENCY_SECONDS)
    )

    ms = int(time.time() * MILLISECONDS_PER_SECOND)
    return {
        "success": True,
        "transaction_id": f"TXN-{user_id}-{ms}",
        "amount": amount,
